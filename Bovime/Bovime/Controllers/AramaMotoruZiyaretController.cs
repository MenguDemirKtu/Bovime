...
{
 public class AramaMotoruZiyaretController : Sayfa
    {
public async Task<ActionResult> Cek( Models.AramaMotoruZiyaretModel modeli)
                            {
                                var nedir = await modeli.ayrintiliAraKos(this);
                                return basariBildirimi("/AramaMotoruZiyaret?id=" + nedir.kodu);
                            }
    public async Task<ActionResult> Index(string id)
        {
try{
            string tanitim = "...";
   tanitim = await Genel.dokumKisaAciklamaKos(this, "AramaMotoruZiyaret"); 
 gorunumAyari("", "", "Ana Sayfa", "/", "/AramaMotoruZiyaret/", tanitim); 
            if (!oturumAcildimi())
...
try{
            ...
            string tanitim = "....";
   tanitim = await Genel.dokumKisaAciklamaKos(this, "AramaMotoruZiyaret"); 
 gorunumAyari("Arama Motoru Ziyaret Kartı", "Arama Motoru Ziyaret Kartı", "Ana Sayfa", "/", "/AramaMotoruZiyaret/", tanitim); 
            enumref_YetkiTuru yetkiTuru = yetkiTuruBelirle(id);
if (await yetkiVarmiKos("AramaMotoruZiyaret", yetkiTuru))
            {
  Models.AramaMotoruZiyaretModel modeli = new Models.AramaMotoruZiyaretModel(); 
...
Models.AramaMotoruZiyaretModel modeli = new Models.AramaMotoruZiyaretModel();
        ...
        [HttpPost]
   public async Task<ActionResult> Kaydet(Models.AramaMotoruZiyaretModel gelen)
        {
            try
            { 
               if (!oturumAcildimi())
