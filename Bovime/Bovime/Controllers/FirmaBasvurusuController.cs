...
{
 public class FirmaBasvurusuController : Sayfa
    {
public async Task<ActionResult> Cek( Models.FirmaBasvurusuModel modeli)
                            {
                                var nedir = await modeli.ayrintiliAraKos(this);
                                return basariBildirimi("/FirmaBasvurusu?id=" + nedir.kodu);
                            }
    public async Task<ActionResult> Index(string id)
        {
try{
            string tanitim = "...";
   tanitim = await Genel.dokumKisaAciklamaKos(this, "FirmaBasvurusu"); 
 gorunumAyari("", "", "Ana Sayfa", "/", "/FirmaBasvurusu/", tanitim); 
            if (!oturumAcildimi())
...
try{
            ...
            string tanitim = "....";
   tanitim = await Genel.dokumKisaAciklamaKos(this, "FirmaBasvurusu"); 
 gorunumAyari("Firma Basvurusu Kartı", "Firma Basvurusu Kartı", "Ana Sayfa", "/", "/FirmaBasvurusu/", tanitim); 
            enumref_YetkiTuru yetkiTuru = yetkiTuruBelirle(id);
if (await yetkiVarmiKos("FirmaBasvurusu", yetkiTuru))
            {
  Models.FirmaBasvurusuModel modeli = new Models.FirmaBasvurusuModel(); 
...
Models.FirmaBasvurusuModel modeli = new Models.FirmaBasvurusuModel();
        ...
        [HttpPost]
   public async Task<ActionResult> Kaydet(Models.FirmaBasvurusuModel gelen)
        {
            try
            { 
               if (!oturumAcildimi())
