...
{
    public class SatisController : Sayfa
    {
        public async Task<ActionResult> Cek(Models.SatisModel modeli)
        {
            var nedir = await modeli.ayrintiliAraKos(this);
            return basariBildirimi("/Satis?id=" + nedir.kodu);
        }
        public async Task<ActionResult> Index(string id)
        {
            ...
            {
                string tanitim = "...";
                tanitim = await Genel.dokumKisaAciklamaKos(this, "Satis");
                gorunumAyari("", "", "Ana Sayfa", "/", "/Satis/", tanitim);
                if (!oturumAcildimi())
        ...
        public async Task<ActionResult> Kart(long id)
        {
            try
            {
                ...
                string tanitim = "....";
                tanitim = await Genel.dokumKisaAciklamaKos(this, "Satis");
                gorunumAyari("Kartı", "Kartı", "Ana Sayfa", "/", "/Satis/", tanitim);
                enumref_YetkiTuru yetkiTuru = yetkiTuruBelirle(id);
                if (await yetkiVarmiKos("Satis", yetkiTuru))
                {
                    Models.SatisModel modeli = new Models.SatisModel();
        ...
        [HttpPost]
        public async Task<ActionResult> Kaydet(Models.SatisModel gelen)
        {
            try
            {
                if (!oturumAcildimi())
