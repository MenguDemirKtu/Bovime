...
{
 public class SikSorulanSorularController : Sayfa
    {
public async Task<ActionResult> Cek( Models.SikSorulanSorularModel modeli)
                            {
                                var nedir = await modeli.ayrintiliAraKos(this);
                                return basariBildirimi("/SikSorulanSorular?id=" + nedir.kodu);
                            }
    public async Task<ActionResult> Index(string id)
        {
try{
            string tanitim = "...";
   tanitim = await Genel.dokumKisaAciklamaKos(this, "SikSorulanSorular"); 
 gorunumAyari("", "", "Ana Sayfa", "/", "/SikSorulanSorular/", tanitim); 
            if (!oturumAcildimi())
...
try{
            ...
            string tanitim = "....";
   tanitim = await Genel.dokumKisaAciklamaKos(this, "SikSorulanSorular"); 
 gorunumAyari("Sık Sorulan Sorular Kartı", "Sık Sorulan Sorular Kartı", "Ana Sayfa", "/", "/SikSorulanSorular/", tanitim); 
            enumref_YetkiTuru yetkiTuru = yetkiTuruBelirle(id);
if (await yetkiVarmiKos("SikSorulanSorular", yetkiTuru))
            {
  Models.SikSorulanSorularModel modeli = new Models.SikSorulanSorularModel(); 
...
Models.SikSorulanSorularModel modeli = new Models.SikSorulanSorularModel();
        ...
        [HttpPost]
   public async Task<ActionResult> Kaydet(Models.SikSorulanSorularModel gelen)
        {
            try
            { 
               if (!oturumAcildimi())
