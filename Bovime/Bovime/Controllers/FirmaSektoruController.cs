...
{
    public class FirmaSektoruController : Sayfa
    {
        public async Task<ActionResult> Cek(Models.FirmaSektoruModel modeli)
        {
            var nedir = await modeli.ayrintiliAraKos(this);
            return basariBildirimi("/FirmaSektoru?id=" + nedir.kodu);
        }
        public async Task<ActionResult> Index(string id)
        {
            ...
            {
                string tanitim = "...";
                tanitim = await Genel.dokumKisaAciklamaKos(this, "FirmaSektoru");
                gorunumAyari("", "", "Ana Sayfa", "/", "/FirmaSektoru/", tanitim);
                if (!oturumAcildimi())
        ...
        public async Task<ActionResult> Kart(long id)
        {
            try
            {
                ...
                string tanitim = "....";
                tanitim = await Genel.dokumKisaAciklamaKos(this, "FirmaSektoru");
                gorunumAyari("Firma Sektörü Kartı", "Firma Sektörü Kartı", "Ana Sayfa", "/", "/FirmaSektoru/", tanitim);
                enumref_YetkiTuru yetkiTuru = yetkiTuruBelirle(id);
                if (await yetkiVarmiKos("FirmaSektoru", yetkiTuru))
                {
                    Models.FirmaSektoruModel modeli = new Models.FirmaSektoruModel();
        ...
        [HttpPost]
        public async Task<ActionResult> Kaydet(Models.FirmaSektoruModel gelen)
        {
            try
            {
                if (!oturumAcildimi())
