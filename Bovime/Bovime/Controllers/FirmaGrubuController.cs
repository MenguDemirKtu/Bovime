...
{
 public class FirmaGrubuController : Sayfa
    {
public async Task<ActionResult> Cek( Models.FirmaGrubuModel modeli)
                            {
                                var nedir = await modeli.ayrintiliAraKos(this);
                                return basariBildirimi("/FirmaGrubu?id=" + nedir.kodu);
                            }
    public async Task<ActionResult> Index(string id)
        {
try{
            string tanitim = "...";
   tanitim = await Genel.dokumKisaAciklamaKos(this, "FirmaGrubu"); 
 gorunumAyari("", "", "Ana Sayfa", "/", "/FirmaGrubu/", tanitim); 
            if (!oturumAcildimi())
...
try{
            ...
            string tanitim = "....";
   tanitim = await Genel.dokumKisaAciklamaKos(this, "FirmaGrubu"); 
 gorunumAyari("Firma Grubu Kartı", "Firma Grubu Kartı", "Ana Sayfa", "/", "/FirmaGrubu/", tanitim); 
            enumref_YetkiTuru yetkiTuru = yetkiTuruBelirle(id);
if (await yetkiVarmiKos("FirmaGrubu", yetkiTuru))
            {
  Models.FirmaGrubuModel modeli = new Models.FirmaGrubuModel(); 
...
Models.FirmaGrubuModel modeli = new Models.FirmaGrubuModel();
        ...
        [HttpPost]
   public async Task<ActionResult> Kaydet(Models.FirmaGrubuModel gelen)
        {
            try
            { 
               if (!oturumAcildimi())
