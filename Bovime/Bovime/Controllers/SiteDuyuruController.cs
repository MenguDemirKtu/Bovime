...
{
    public class SiteDuyuruController : Sayfa
    {
        public async Task<ActionResult> Cek(Models.SiteDuyuruModel modeli)
        {
            var nedir = await modeli.ayrintiliAraKos(this);
            return basariBildirimi("/SiteDuyuru?id=" + nedir.kodu);
        }
        public async Task<ActionResult> Index(string id)
        {
            ...
            {
                string tanitim = "...";
                tanitim = await Genel.dokumKisaAciklamaKos(this, "SiteDuyuru");
                gorunumAyari("", "", "Ana Sayfa", "/", "/SiteDuyuru/", tanitim);
                if (!oturumAcildimi())
        ...
        public async Task<ActionResult> Kart(long id)
        {
            try
            {
                ...
                string tanitim = "....";
                tanitim = await Genel.dokumKisaAciklamaKos(this, "SiteDuyuru");
                gorunumAyari("Site Duyuru Kartı", "Site Duyuru Kartı", "Ana Sayfa", "/", "/SiteDuyuru/", tanitim);
                enumref_YetkiTuru yetkiTuru = yetkiTuruBelirle(id);
                if (await yetkiVarmiKos("SiteDuyuru", yetkiTuru))
                {
                    Models.SiteDuyuruModel modeli = new Models.SiteDuyuruModel();
        ...
        [HttpPost]
        public async Task<ActionResult> Kaydet(Models.SiteDuyuruModel gelen)
        {
            try
            {
                if (!oturumAcildimi())
