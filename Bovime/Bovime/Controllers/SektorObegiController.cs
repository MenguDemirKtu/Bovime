...
{
    public class SektorObegiController : Sayfa
    {
        public async Task<ActionResult> Cek(Models.SektorObegiModel modeli)
        {
            var nedir = await modeli.ayrintiliAraKos(this);
            return basariBildirimi("/SektorObegi?id=" + nedir.kodu);
        }
        public async Task<ActionResult> Index(string id)
        {
            ...
            {
                string tanitim = "...";
                tanitim = await Genel.dokumKisaAciklamaKos(this, "SektorObegi");
                gorunumAyari("", "", "Ana Sayfa", "/", "/SektorObegi/", tanitim);
                if (!oturumAcildimi())
        ...
        public async Task<ActionResult> Kart(long id)
        {
            try
            {
                ...
                string tanitim = "....";
                tanitim = await Genel.dokumKisaAciklamaKos(this, "SektorObegi");
                gorunumAyari("Sektör Öbeği Kartı", "Sektör Öbeği Kartı", "Ana Sayfa", "/", "/SektorObegi/", tanitim);
                enumref_YetkiTuru yetkiTuru = yetkiTuruBelirle(id);
                if (await yetkiVarmiKos("SektorObegi", yetkiTuru))
                {
                    Models.SektorObegiModel modeli = new Models.SektorObegiModel();
        ...
        [HttpPost]
        public async Task<ActionResult> Kaydet(Models.SektorObegiModel gelen)
        {
            try
            {
                if (!oturumAcildimi())
