...
{
    public class PaketController : Sayfa
    {
        public async Task<ActionResult> Cek(Models.PaketModel modeli)
        {
            var nedir = await modeli.ayrintiliAraKos(this);
            return basariBildirimi("/Paket?id=" + nedir.kodu);
        }
        public async Task<ActionResult> Index(string id)
        {
            ...
            {
                string tanitim = "...";
                tanitim = await Genel.dokumKisaAciklamaKos(this, "Paket");
                gorunumAyari("", "", "Ana Sayfa", "/", "/Paket/", tanitim);
                if (!oturumAcildimi())
        ...
        public async Task<ActionResult> Kart(long id)
        {
            try
            {
                ...
                string tanitim = "....";
                tanitim = await Genel.dokumKisaAciklamaKos(this, "Paket");
                gorunumAyari("Paket Kartı", "Paket Kartı", "Ana Sayfa", "/", "/Paket/", tanitim);
                enumref_YetkiTuru yetkiTuru = yetkiTuruBelirle(id);
                if (await yetkiVarmiKos("Paket", yetkiTuru))
                {
                    Models.PaketModel modeli = new Models.PaketModel();
        ...
        [HttpPost]
        public async Task<ActionResult> Kaydet(Models.PaketModel gelen)
        {
            try
            {
                if (!oturumAcildimi())
