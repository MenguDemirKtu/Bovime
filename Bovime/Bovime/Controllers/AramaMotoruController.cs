...
{
 public class AramaMotoruController : Sayfa
    {
public async Task<ActionResult> Cek( Models.AramaMotoruModel modeli)
                            {
                                var nedir = await modeli.ayrintiliAraKos(this);
                                return basariBildirimi("/AramaMotoru?id=" + nedir.kodu);
                            }
    public async Task<ActionResult> Index(string id)
        {
try{
            string tanitim = "...";
   tanitim = await Genel.dokumKisaAciklamaKos(this, "AramaMotoru"); 
 gorunumAyari("", "", "Ana Sayfa", "/", "/AramaMotoru/", tanitim); 
            if (!oturumAcildimi())
...
try{
            ...
            string tanitim = "....";
   tanitim = await Genel.dokumKisaAciklamaKos(this, "AramaMotoru"); 
 gorunumAyari("Arama Motoru Kartı", "Arama Motoru Kartı", "Ana Sayfa", "/", "/AramaMotoru/", tanitim); 
            enumref_YetkiTuru yetkiTuru = yetkiTuruBelirle(id);
if (await yetkiVarmiKos("AramaMotoru", yetkiTuru))
            {
  Models.AramaMotoruModel modeli = new Models.AramaMotoruModel(); 
...
Models.AramaMotoruModel modeli = new Models.AramaMotoruModel();
        ...
        [HttpPost]
   public async Task<ActionResult> Kaydet(Models.AramaMotoruModel gelen)
        {
            try
            { 
               if (!oturumAcildimi())
