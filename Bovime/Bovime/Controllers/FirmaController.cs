...
{
    public class FirmaController : Sayfa
    {
        public async Task<ActionResult> Cek(Models.FirmaModel modeli)
        {
            var nedir = await modeli.ayrintiliAraKos(this);
            return basariBildirimi("/Firma?id=" + nedir.kodu);
        }
        public async Task<ActionResult> Index(string id)
        {
            ...
            {
                string tanitim = "...";
                tanitim = await Genel.dokumKisaAciklamaKos(this, "Firma");
                gorunumAyari("", "", "Ana Sayfa", "/", "/Firma/", tanitim);
                if (!oturumAcildimi())
        ...
        public async Task<ActionResult> Kart(long id)
        {
            try
            {
                ...
                string tanitim = "....";
                tanitim = await Genel.dokumKisaAciklamaKos(this, "Firma");
                gorunumAyari("Firma Kartı", "Firma Kartı", "Ana Sayfa", "/", "/Firma/", tanitim);
                enumref_YetkiTuru yetkiTuru = yetkiTuruBelirle(id);
                if (await yetkiVarmiKos("Firma", yetkiTuru))
                {
                    Models.FirmaModel modeli = new Models.FirmaModel();
        ...
        [HttpPost]
        public async Task<ActionResult> Kaydet(Models.FirmaModel gelen)
        {
            try
            {
                if (!oturumAcildimi())
